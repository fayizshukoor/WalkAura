...
export const downloadInvoice = asyncHandler(async (req, res) => {
  ...
  if (!order || order.payment.status !== "PAID") {
    return res
      .status(404)
      .json({ success: false, message: "Invoice unavailable" });
  }
  ...
  doc
    ...
    .font("Roboto-Bold")
    .fontSize(20)
    .text("WALKAURA", SAFE_X, 50);
  doc
    .fillColor("#666")
    .font("Roboto-Regular")
    .fontSize(8)
    .text("PREMIUM FOOTWEAR & LIFESTYLE", SAFE_X, 72);
  doc
    .fillColor(primaryColor)
    .font("Roboto-Bold")
    .fontSize(24)
    .text("INVOICE", 350, 50, { align: "right", width: 205 });
  ...
  doc
    ...
    .font("Roboto-Bold")
    .fontSize(8)
    .text("BILLED TO", SAFE_X, infoY);
  doc
    .fillColor(primaryColor)
    .font("Roboto-Bold")
    .fontSize(10)
    .text(invoice.customer.name, SAFE_X, infoY + 12);
  doc
    .fillColor("#444")
    .font("Roboto-Regular")
    .fontSize(8)
  ...
  doc
    ...
    .font("Roboto-Bold")
    .fontSize(8)
    .text("SHIPPED TO", shipX, infoY);
  doc
    .fillColor(primaryColor)
    .font("Roboto-Bold")
    .fontSize(9)
    .text(addr.fullName || invoice.customer.name, shipX, infoY + 12);
  ...
  doc
    ...
    .font("Roboto-Bold")
    .fontSize(8)
    .text("INVOICE DETAILS", 380, infoY, { align: "right", width: 175 });
  doc
    .fillColor(primaryColor)
    .font("Roboto-Regular")
    .fontSize(8)
    .text(`Invoice No: ${invoice.invoiceNumber}`, 380, infoY + 12, {
      align: "right",
      width: 175,
    })
  ...
  invoice.items.forEach((item) => {
    ...
    doc.text(item.quantity.toString(), 360, itemY, {
      width: 30,
      align: "center",
    });
    doc.text(`${rupee}${item.price.toLocaleString()}`, 410, itemY, {
      width: 65,
      align: "right",
    });
    doc.text(`${rupee}${item.total.toLocaleString()}`, 485, itemY, {
      width: 65,
      align: "right",
    });
  ...
  if (itemsRendered < invoice.items.length) {
    doc
      ...
      .fontSize(7)
      .text(
        `* Showing ${itemsRendered} of ${invoice.items.length} items.`,
        SAFE_X,
        itemY + 2,
      );
  }
  ...
  const drawRow = (label, value) => {
    ...
    doc.text(`${rupee}${value.toLocaleString()}`, valueX, currentSummaryY, {
      width: 65,
      align: "right",
    });
    currentSummaryY += 12;
  ...
  doc.text(
    `${rupee}${invoice.pricing.totalAmount.toLocaleString()}`,
    475,
    totalBarY + 10,
